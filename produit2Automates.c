...
{
    Automate automProduit;
    int tailleProduit[4] = {0};
    bool isSameApha = true;
    ...
    isSameApha = produit_Automate(*autom0, taille0, autom1, taille1, EDepInit, tailleEDepInit, 1, &automProduit, tailleProduit);
    ...
    if(isSameApha)
        genererDotImg(&automProduit, tailleProduit, "Automate_Produit");

    if(isSameApha && changerAutomate())
    {
        printf("***** ON TRAVAILLE MAINTENANT AVEC LE PRODUIT DE 2 AUTOMATES *****\n");
        copyAutomate(autom0, taille0, automProduit, tailleProduit);
        return true;
    }
...
bool produit_Automate(Automate autom0, int *taille0, Automate autom1, int *taille1, int **EDep, int tailleEDep[2], int firstTime, Automate *automProduit, int *tailleProduit)
{
    ...
    {
        printf("\n#####-- Ces 2 deux automates n'ont pas le meme alphabet. Veuillez verifier --#####\nAny key to continue...");
        getch();
        return false;
    }
