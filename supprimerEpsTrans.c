        ...
        for(int j=0; j<taille[0] ; j++)
        {
            ...
            */ 
            ...
            {          
                ...
                else if(etat == EDep[i]  && autom->transition[j].etiquete[0] != '`')
                {
...
void automateSansEpsilone(Automate *autom, int *taille)
{
    ...
    if(file!=NULL && fileRead != NULL)
    {
        ...
        nowAutom = getAutomateAllAccessible(nowAutom, nouvTaille);
        genererDotImg(&nowAutom, nouvTaille, "Automate_Sans_Epsilon");  
        
        //changer l'automate  init
        if(changerAutomate())
            copyAutomate(autom, taille, nowAutom, nouvTaille);
    }
    else
    {
...
Automate getAutomateAllAccessible(Automate autom, int taille[]) //suppression des étates inatteignables
{
    FILE *fileTemp = fopen("automTemp.txt", "a");
